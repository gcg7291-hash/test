...
// 슬라이스 생성 -> 리듀서 와 액션을 생성
const counterSlice = createSlice({
  name: "counter",
  initialState: initialState, // 초기 상태 정의 속성
  // 리듀서와 액션 정의
  reducers: {
    // 상태 count를 1증가  리듀서, 액션 작성
    increment: (state) => {
      // 매개변수 state : 현재 슬라이스에 정의된 상태를 저장  state: count: 상태_값

      state["count"] = state["count"] + 1; // 불변성 x, 상태를 직접 수정
      // newState = {...state, count: state["count"] + 1 }
    },

    // 상태 count 를 특정 값 (외부에서 인자로 전달한) 만큼 증가시키는 리듀서와 액션
...
// export cont increment = counterslice.actions.increment
